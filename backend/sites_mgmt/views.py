...
class PublicAuditPDFView(APIView):
    """GET /api/public/audit/<domain>/pdf/

    Returns a PDF rendering of the public audit for `domain`. If the audit is
    cached we render from cache (no extra API spend); otherwise we ask the
    visitor to run the audit first via the SPA (faster than re-running
    server-side from a download click and avoids hidden API costs).

    Designed for the cold-outreach use case: Darius runs an audit via the
    SPA, downloads the PDF, attaches it to a LinkedIn DM along with a
    personalized note. The PDF ends with a CTA to gridar.app/login.
    """
    authentication_classes = []
    permission_classes = []
    throttle_classes = [PublicAuditThrottle]
    ...
    def get(self, request, domain):
        ...
        try:
            ...
            return Response(
                {'error': 'PDF generation indisponible (xhtml2pdf manquant).'},
                status=status.HTTP_503_SERVICE_UNAVAILABLE,
            )

        normalized = _normalize_audit_domain(domain)
        if not normalized:
            return Response(
        ...
        # Pull the cached audit. We don't auto-run an audit from a download
        # link because that would let bots burn through our API quota by
        # crawling random domains. If the audit isn't cached, ask the user
        # to run it first via the SPA.
        cached = cache.get(f'public-audit:v2:{normalized}')
        if not cached:
            return Response(
                {'error': 'Lance d\'abord un audit sur gridar.app/audit, puis reviens telecharger le PDF.'},
                status=status.HTTP_404_NOT_FOUND,
            )

        # Map score to a color hex used by the template's CSS.
        score = cached.get('composite_score')
        if score is None:
            score_color = '#888888'
            score_label = 'Donnees insuffisantes'
        elif score >= 80:
            score_color = '#10b981'
            score_label = 'Tres bon' if score < 90 else 'Excellent'
        elif score >= 60:
            score_color = '#f59e0b'
            score_label = 'Correct'
        elif score >= 40:
            score_color = '#f59e0b'
            score_label = 'A ameliorer'
        else:
            score_color = '#dc2626'
            score_label = 'Critique'

        ps = cached.get('pagespeed') or {}
        crawl = cached.get('crawl') or {}

        ctx = {
            'domain': normalized,
            'audited_at': (cached.get('audited_at') or '')[:10],
            'composite_score': score,
            'score_color': score_color,
            'score_label': score_label,
            'title': crawl.get('title') or '',
            'meta_description': crawl.get('meta_description') or '',
            'pagespeed_avg': ps.get('avg'),
            'pagespeed_performance': ps.get('performance'),
            'pagespeed_seo': ps.get('seo'),
            'keywords': cached.get('top_keywords_estimated') or [],
            'recos': cached.get('recos_partial') or [],
        }

        html = render_to_string('public_audit_pdf.html', ctx)
        ...
        resp['Content-Disposition'] = (
            f'attachment; filename="gridar-audit-{normalized}.pdf"'
        )
        return resp
