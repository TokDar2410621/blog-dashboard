    ...
    <Card>
      <CardHeader>
        <CardTitle className="text-lg flex items-center gap-2">
          ...
          Sante hreflang multi-langue
        </CardTitle>
        <p className="text-sm text-muted-foreground">
          Verifie que tes articles ont bien leurs traductions publiees dans toutes les langues du site.
        </p>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        {isError && (
          <p className="text-sm text-muted-foreground">
            Impossible de charger l&apos;analyse hreflang.
          </p>
        )}
            ...
            {data.groups_incomplete.length > 0 && (
              <div className="space-y-2">
                <h4 className="text-xs font-mono uppercase tracking-wide text-muted-foreground">
                  Groupes incomplets (langues manquantes)
                </h4>
                <ul className="space-y-2 max-h-64 overflow-y-auto">
                  {data.groups_incomplete.slice(0, 20).map((g) => (
                    ...
                    >
                      <Link
                        href={`${base}/articles/${g.sample_slug}`}
