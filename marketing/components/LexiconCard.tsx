    ...
    <Card>
      <CardHeader>
        <CardTitle className="text-lg flex items-center gap-2">
          ...
          Lexique québécois (FR-CA)
        </CardTitle>
        <p className="text-xs text-muted-foreground">
          Détecte les termes France (shopping, week-end, parking, email...) et
          propose les équivalents québécois recommandés par l&apos;OQLF.
        </p>
      </CardHeader>
      <CardContent className="space-y-3">
        {isLoading || !data ? (
          ...
          <p className="text-sm text-green-600 flex items-center gap-2">
            ✓ Aucun terme franco-français détecté. Ton article respecte le
            lexique québécois.
          </p>
        ) : (
          <>
            <p className="text-xs text-muted-foreground">
              {data.unique_terms} terme(s) à remplacer pour un public québécois
              ({data.total_matches} occurrence(s) au total).
            </p>
