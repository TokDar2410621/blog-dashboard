...
const ACTION_LABELS: Record<DecayingPage["suggested_action"], string> = {
  redirect_or_remove: "Redirect/Suppr",
  major_refresh: "Refresh majeur",
  minor_refresh: "Refresh mineur",
};
    ...
    <div className="space-y-6 max-w-5xl">
      ...
      <div>
        <h1 className="text-2xl font-bold">
          {"Détection de déclin de contenu"}
        </h1>
        <p className="text-muted-foreground">
          {"Compare les impressions et clics GSC sur les 30 derniers jours vs les 30 jours d'avant pour détecter les articles qui perdent en traction."}
        </p>
      </div>
        ...
        <CardContent>
          <div className="flex flex-wrap items-end gap-3">
            <div className="space-y-2">
              <label className="text-sm font-medium">{"Fenêtre"}</label>
          ...
          {mutation.isPending && (
            <p className="text-xs text-muted-foreground mt-3">
              {"Deux requêtes Google Search Console (période courante + période précédente). 5-15 secondes selon le nombre d'articles."}
            </p>
          )}
        </CardContent>
      </Card>
      ...
      {errorCode === "gsc_not_configured" && (
        <Card>
          <CardContent className="py-6 text-center text-sm">
            <p className="mb-3">
              {"GSC n'est pas configuré pour ce site. Configure l'URL de propriété GSC dans les paramètres."}
            </p>
      ...
      {errorCode === "gsc_reauth_required" && (
        <Card>
          <CardContent className="py-6 text-center text-sm">
            <p className="mb-3">
              {"Token GSC expiré. Reconnecte-toi à Google Search Console dans les paramètres."}
            </p>
            <Link href={`${base}/parametres/gsc`}>
              <Button variant="outline">
                {"Aller aux paramètres"}
        ...
        <>
          ...
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <p className="text-xs text-muted-foreground">
              {`Comparaison : ${data.period_current.start} - ${data.period_current.end} vs ${data.period_previous.start} - ${data.period_previous.end}`}
            </p>
            <ExportButton
          ...
          {data.decaying.length === 0 ? (
            <Card>
              <CardContent className="py-12 text-center text-muted-foreground">
                ...
                <p>{"Aucun déclin significatif détecté. Bravo !"}</p>
              </CardContent>
            </Card>
          ) : (
            <>
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                        ...
                        <div className="shrink-0 flex flex-col items-start sm:items-end gap-2">
                          ...
                          >
                            ...
                            {ACTION_LABELS[page.suggested_action]}
                          </span>
                          {lift > 0 && (
                          ...
                          <div className="flex items-center gap-2">
                            <Link href={`${base}/articles/${page.slug}`}>
                            ...
                            <Link
                              href={`${base}/generer?slug=${encodeURIComponent(
                                page.slug,
