...
export function ContentBriefPanel({ language, defaultKeyword = "", onApply }: Props) {
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      if (!res.ok) {
        ...
        throw new Error(err.error || "Erreur lors de la génération du brief");
      }
  ...
  const handleGenerate = () => {
    if (!keyword.trim()) {
      toast.error("Le mot-clé cible est requis");
      return;
    }
    mutation.mutate();
  };
  ...
  const apply = () => {
    ...
    toast.success("Brief appliqué au formulaire");
  };
    ...
    <Card>
      <CardHeader>
        ...
        <p className="text-sm text-muted-foreground">
          Génère un plan stratégique avant d&apos;écrire - intent, titres, plan, FAQ, entités, schemas, signaux EEAT.
        </p>
      </CardHeader>
            ...
            {open && (
              <div className="space-y-4 border-t pt-4">
                ...
                {data.serp_competitors.length > 0 && (
                  <div>
                    <h4 className="text-xs font-mono uppercase tracking-wide text-muted-foreground mb-2">
                      Concurrents SERP ({data.serp_competitors.length})
                    </h4>
