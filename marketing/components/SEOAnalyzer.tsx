...
export function SEOAnalyzer({
  ...
  // Dashboard UI language (the SPA used react-i18next; FR hardcoded here).
  ...
  const checks = useMemo<SEOCheck[]>(() => {
    ...
    // avoid false positives ("SEO" is not "Seoul") and false negatives
    ...
    return results;
  }, [title, excerpt, content, slug, coverImage, keyword, synonyms, lang]);
  ...
  const handleAiAudit = async () => {
    ...
    try {
      ...
      toast.error(lang === "fr" ? "Erreur audit IA" : "AI audit error");
    } finally {
  ...
  const handleFixSingle = async (issue: string, kind: "weakness" | "action") => {
    ...
    try {
      const { authFetch } = await import("@/lib/api-client");
      const articleLang = language || lang || "fr";
      ...
      toast.success(
        lang === "fr" ? "Correction appliquée" : "Fix applied"
      );
    } catch {
      toast.error(
        lang === "fr" ? "Erreur correction" : "Fix error"
      );
    } finally {
      setFixingItem(null);
    }
  ...
  const handleFixAll = async () => {
    ...
    try {
      ...
      toast.success(lang === "fr" ? "Corrections appliquées !" : "Fixes applied!");
    } catch {
      toast.error(lang === "fr" ? "Erreur correction IA" : "AI fix error");
    } finally {
      setFixLoading(false);
    }
  };
  ...
  const copyKeyword = async (kw: string) => {
    try {
      ...
      toast.success(lang === "fr" ? "Copie !" : "Copied!");
    } catch {
      toast.error(lang === "fr" ? "Erreur copie" : "Copy error");
    }
  };
  ...
  const copyMarkdownLink = (anchor: string, linkSlug: string) => {
    ...
    toast.success(
      lang === "fr" ? "Markdown copie!" : "Markdown copied!"
    );
  };
  ...
  const handleBacklinks = async () => {
    ...
    try {
      ...
      setBacklinksError(
        lang === "fr" ? "Erreur reseau" : "Network error"
      );
    } finally {
  ...
  const runPageSpeed = async (strategy: "mobile" | "desktop") => {
    ...
    if (!target) {
      toast.error(
        lang === "fr" ? "URL requise" : "URL required"
      );
      return;
    }
    ...
    try {
      ...
      toast.error(
        lang === "fr" ? `Erreur PageSpeed: ${msg}` : `PageSpeed error: ${msg}`
      );
    } finally {
      setPsiLoading(null);
    }
            ...
            <div className="flex items-center gap-2">
              {hasIssues && onApplyFix && (
                ...
                >
                  ...
                  {lang === "fr" ? "Corriger avec IA" : "Fix with AI"}
                </Button>
              )}
              <Button
                variant="ghost"
        ...
        <CardContent className="px-4 pb-4 space-y-3">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            >
              ...
              {lang === "fr" ? "Tester mobile" : "Test mobile"}
            </Button>
            <Button
              size="sm"
              variant="outline"
            ...
            >
              ...
              {lang === "fr" ? "Tester desktop" : "Test desktop"}
            </Button>
          </div>
            ...
            <div className="space-y-3 pt-2 border-t">
              ...
              <div className="grid grid-cols-3 gap-2">
                ...
                <div className="flex flex-col items-center gap-1">
                  ...
                  <p className="text-[10px] text-muted-foreground">
                    {lang === "fr" ? "Accessibilite" : "Accessibility"}
                  </p>
                </div>
              </div>
            </div>
        ...
        <Card>
          <CardHeader className="py-3 px-4">
            <CardTitle className="text-sm flex items-center gap-2">
              ...
              {lang === "fr" ? "Cannibalisation" : "Cannibalization"}
            </CardTitle>
          </CardHeader>
      ...
      <Card>
        <CardHeader className="py-3 px-4">
          <CardTitle className="text-sm flex items-center gap-2">
            ...
            {lang === "fr" ? "Audit SEO par IA" : "AI SEO Audit"}
          </CardTitle>
        </CardHeader>
        <CardContent className="px-4 pb-4 space-y-3">
          <Button
              ...
              {aiAudit.weaknesses?.length > 0 && (
                <div>
                  <p className="text-xs font-semibold mb-1 text-red-500">
                    ✗ {lang === "fr" ? "Points faibles" : "Weaknesses"}
                  </p>
              ...
              {aiAudit.actions?.length > 0 && (
                <div>
                  <p className="text-xs font-semibold mb-1 text-primary">
                    → {lang === "fr" ? "Actions à faire" : "Action items"}
                  </p>
                  <ul className="text-xs space-y-1.5">
                ...
                <div className="pt-2 border-t">
                  ...
                  >
                    {fixLoading ? (
                      <>
                        <Loader2 className="h-4 w-4 mr-1.5 animate-spin" />
                        {lang === "fr" ? "Correction en cours..." : "Fixing..."}
                      </>
                    ) : (
                      <>
                        <Wand2 className="h-4 w-4 mr-1.5" />
              ...
              {backlinks.top_domains.length > 0 && (
                <div>
                  <p className="text-xs font-semibold mb-1">
                    {lang === "fr" ? "Top domaines" : "Top domains"}
                  </p>
        ...
        <CardContent className="px-4 pb-4 space-y-3">
          ...
          >
            {linkLoading ? (
              ...
              <>
                <LinkIcon className="h-4 w-4 mr-1.5" />
                {lang === "fr" ? "Suggerer des liens" : "Suggest links"}
              </>
            )}
          </Button>
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs mt-0.5">
                        ...
                        {siteId ? (
                          <Link
                            href={`/dashboard/${siteId}/articles/${s.slug}`}
